# -*- coding: utf-8 -*-
"""
Created on Mon Feb 17 16:44:46 2025
...
def monte_carlo_simulation_gbm(ticker, num_simulations, num_days, initial_price):
    # Fetch historical data for the ticker using yfinance
    data = yf.download(ticker, period="10y", interval="1d")
    
    # Calculate the log returns for GBM
    ...
    for i in range(num_simulations):
        ...
        for j in range(1, num_days):
            random_shock = np.random.normal(0, 1)  # Standard normal random shock
            price_change = (daily_return_mean - 0.5 * daily_return_std**2) + daily_return_std * random_shock
...
for ticker in tickers:
    ...
    simulations, expected_value = monte_carlo_simulation_gbm(ticker, num_simulations, num_days, latest_price)
